...
## Bug 日志
- ProTable背景颜色问题
- 在ProTable中若修改管理员自己的信息，则自身profile等不会发生改变
   - 这里逻辑正确，通常如果管理员或者后台修改了自己的信息，则需要重新登陆才能生效，自己修改的本身额用setUser已经实现更新并存到localStrotage里了
- layout本身的报错，Nextjs的小助手的各种报错
   - 目前已经无报错
- 用户profile本身的各种显示问题
   - 目前一切正常
- 用户信息页面加载前会出现页面布局错乱问题
   - 修改后分步加载，目前已无异常

## 2025/06/05
 ### 用户个人信息的更新流程
 - 用户登录后取回用户信息保存在localstrotage里，保存后AuthContext会在进入应用时读取一次， 后来可以用setUser方法更新用户信息，user来调取用户信息
 - 用户列表加载慢的问题
    - 先用提示加载中请稍后进行解决，后续继续进行优化，主要是后台响应问题
 - 优化用户界面，等待时进行相关提示 (已完成)
	- 登录时提示登录中
	- 登出时提示正在登出
	- 注册后显示注册成功并且跳到登录页面
 - 用户登录态管理，userRole为0的用户看不到用户管理界面
	- 已完成，在sidebar里判断userRole然后是否显示